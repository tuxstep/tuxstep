...
| 17 | Linux app compatibility: `pkgwrap` self-contained bundles (from Gershwin) | Existing glibc-based Linux apps run via self-contained `.app` bundles produced by Gershwin's `pkgwrap` tool. A bundle contains the binary, its full dependency closure, and a libsystem-linked launcher that exec's the binary through the bundle's own dynamic linker. Bundles install under `/Local/Applications/` and are isolated from `/System` — no pollution, no conflicts between apps, no package manager required. |
...
- 2026-04-26 (rev 3): Boot architecture, drag-and-drop deployment, and multi-arch commitment locked in. Decisions #11 (bootloader: `tuxstep/grub` fork with silent boot + Mac-style boot keys), #14 (`/System` is the OS, drag-and-drop deployable via auto-bless), #15 (i386 + amd64 + arm64 all required from v0.1), #16 (filesystem: ext4), #17 (Linux app compat via Gershwin's `pkgwrap`). Boot chain rewritten for the hidden FAT32 ESP + GRUB chainloader + EFI-stub kernel pattern. New "Volume management" section documents `newfs` / `bless` / auto-bless daemon. Phase plan extended with Phases 6-9 (post-v0.1). Component repos table extended with `tuxstep/grub` and `tuxstep/bless`.
