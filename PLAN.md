...
- 2026-04-26 (rev 3): Boot architecture, drag-and-drop deployment, and multi-arch commitment locked in. Decisions #11 (bootloader: `tuxstep/grub` fork with silent boot + Mac-style boot keys), #14 (`/System` is the OS, drag-and-drop deployable via auto-bless), #15 (i386 + amd64 + arm64 all required from v0.1), #16 (filesystem: ext4). Boot chain rewritten for the hidden FAT32 ESP + GRUB chainloader + EFI-stub kernel pattern. New "Volume management" section documents `newfs` / `bless` / auto-bless daemon. Phase plan extended with Phases 6-9 (post-v0.1). Component repos table extended with `tuxstep/grub` and `tuxstep/bless`.
